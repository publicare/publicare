...
//        return " and (".$_page->_db->nomes_tabelas["objeto"].".data_publicacao<=".date("YmdHi")."00 and ".$_page->_db->nomes_tabelas["objeto"].".data_validade>=".date("YmdHi")."00)";
     ...
     */
    function Search(&$_page, $query, $excecoes="", $parentesco_excecoes="", $pagina=1, $paginacao=20, $prefixos_classes="")
    {
        ...
        if ((isset($query) && strlen($query)>1))
        {
            ...
            $sql = "select distinct(o.cod_objeto), 
                    ...
                    where
                                o.cod_status = 2
                                and o.apagado = 0 ";
            if ($prefixos_classes!="") $sql .= "and c.prefixo in (".$prefixos_classes.") ";
    ...
    function CodigoDasClasses(&$_page, $classes)
    {
        ...
        else
        {
            ...
            {
                if (isset($_SESSION['classesLocalizarPorNome'][strtolower($nome)])) $saida[] = $_SESSION['classesLocalizarPorNome'][strtolower($nome)];
                else $saida[] = $_SESSION['classesPrefixos'][strtolower($nome)];
            }
        }
        return $saida;
    }
        ...
        if ((!isset($_SESSION['classesPrefixos'])) || (!is_array($_SESSION['classesPrefixos'])) || ($_page->_usuario->EstaLogado()))
        {
            ...
            if ($rs->_numOfRows > 0){
                while (!$rs->EOF){
                    ...
                    $_SESSION['classesNomes'][mb_strtolower($rs->fields['nome'], "UTF-8")]=$rs->fields['cod_classe'];
                    $_SESSION['classesLocalizarPorNome'][mb_strtolower($rs->fields['nome'], "UTF-8")]=$rs->fields['cod_classe'];

                    if ($rs->fields['indexar'])
                    {
                        if (!isset($_SESSION['classesIndexaveis']) || !in_array($rs->fields['cod_classe'], $_SESSION['classesIndexaveis'])) $_SESSION['classesIndexaveis'][]=$rs->fields['cod_classe'];
                    }
                    $rs->MoveNext();
                }
            }
...
//                $prop = preg_replace("[\(|\)]", "", $prop);
	...
	function ExecutaScript(&$_page, $codClasse, $codPele,$codTexto) {
		...
		if (!is_null($codPele) && $codPele > 0 && file_exists($_SERVER['DOCUMENT_ROOT']."/html/execscript/exec_".$PeleUtilizada['prefixo']."_".$ClasseUtilizada['prefixo']."_".$codTexto.".php"))	{
