...
?>
	...
	<!--
		...
		{
			if (<? echo $classinfo['objcount']+0?>)
				return (confirm ("Tem certeza que quer apagar a classe?\nSeu portal contem <? echo $classinfo['objcount']?> objetos dessa classe\n que SERAO APAGADOS TAMBEM!"));
			else
				return (confirm ("Tem certeza que quer apagar a classe?"));
		}
		...
		function EstaEntre(campo,elm,conjunto)
		{
			...
			for (f=0;f<conjunto.length;f++)
			{
				...
				{
					alert(campo+" "+elm+" j� existente. Por favor digite outro.");
					return true;
				}
			}
			return false;
            		...
            		<tr>
            			...
            			<td class="pblTextoForm">
            				<select class="pblSelectForm" name="cod_classe">
            					...
            					<? echo $_page->_administracao->DropDownClasses($_page, $_GET['cod_classe'],false);?>
            				</select>
               ...
               <script>
               	function ChecaValidade()
               	{
					prfx= new Array('<?echo $strprfx?>');
               		...
               		if (EstaEntre('Prefixo',document.forms['editclass'].prefixo.value,prfx))
               			return false;
               		if (EstaEntre('Nome',document.forms['editclass'].nome.value,cls))
               			return false;
               		for (f=0;f < ArrayCheck.length;f++)
               		{
               			if (ArrayCheck[f]==1)
               			{
               				//alert(f+':'+typeof(document.forms['editclass']['prop_'+f+'_cod_referencia_classe']));
               				if (typeof(document.forms['editclass']['prop_'+f+'_cod_referencia_classe'])=='object')
               				{
               					if (document.forms['editclass']['prop_'+f+'_cod_referencia_classe'].value==0)
               					{
               						alert ("Campo \'Classe de Refer�ncia\' da Propriedade "+f+" n�o pode ficar em branco.");
               						return false;
               					}
               					if (document.forms['editclass']['prop_'+f+'_campo_ref'].value=='')
               					{
               						alert ("Campo \'Campo de Refer�ncia\' da Propriedade "+f+" n�o pode ficar em branco.");
               						return false;
               					}
               				}
               				else
               				{
               					//alert(f+':'+document.forms['editclass']['prop_'+f+'_cod_referencia_classe'].value);
               				}
               			}
               		}
               		...
               		for (f=0;f<conta_prop;f++)
               		{
               		    if (typeof(document.forms['editclass']['prop_'+f+'_nome'])=='object')
               			{
               				...
               				{
               					alert ("Campo \'Nome\' da Propriedade "+f+" n�o pode ficar em branco");
               					return false;
               				}
               			}
               		}
				...
				<input type="hidden" name="old_indexar" value="<? echo $classinfo['classe']['indexar']?>">
               	...
               	<table border=0 width="100%" cellpadding="3" cellspacing="2">
               		<tr>
						<td width="140" class="pblTextoLabelForm" valign="top" valign="top">
               			...
               			<td class="pblTextoForm" valign="top" valign="top">
               				<input class="pblInputForm" type="text" name="nome" value="<? echo $classinfo['classe']['nome']?>">
               			</td>
               		</tr>
               		<tr>
               			...
               			<td class="pblTextoForm" valign="top">
               				<input class="pblInputForm" type="text" name="prefixo" value="<? echo $classinfo['classe']['prefixo'];?>" 
               		...
               		<tr>
               			...
               			<td class="pblTextoForm" valign="top">
               				<textarea class="pblInputForm" rows="4" cols="<? echo $areasize?>" name="descricao"><? echo $classinfo['classe']['descricao']?></textarea>
               			</td>
               		</tr>
               		<tr>
               			<td class="pblTextoLabelForm" valign="top">
               			...
               			<td class="pblTextoForm" valign="top">
               			   ...
               			   {
               			   		if ($classinfo['classe']['temfilhos']) echo "Sim";
					...
					<tr>
               			...
               			<td class="pblTextoForm" valign="top">
               			   ...
               			   {
               			   		if ($classinfo['classe']['indexar']) echo "Sim";
               					else echo "N&atilde;o";
							?>
								<input type="hidden" name="indexar" value="<? echo $classinfo['classe']['indexar']?>">
							<?
               			   }?>
               			</td>
               		</tr>
				...
				<tr>
					...
					<tr>
               			<td class="pblTextoForm" valign="top" id="tdPodeConterChk">
               				<? echo $_page->_administracao->CheckBoxClasses($_page, 'podeconter[]', $classinfo['todas'], 'permitido', ($classinfo['classe']['temfilhos']));?>
               			</td>
               			<td class="pblTextoForm" valign="top" id="tdPodeSerCriadoChk">
               				<? echo $_page->_administracao->CheckBoxClasses($_page, 'criadoem[]',$classinfo['todas'],'criadoem');?>
               			</td>
				</tr>
			    </table>
			    </td>
...
//							exit();
							...
							?>
							<input class="pblInputForm" type="text" name="incluirObjeto" value="<? echo $_GET['IncluirObjeto']?>"><P>
							<?
								if ($_GET['MsgObjeto'])
								{
               	...
               	<div id="pp">
               		<?  if (is_array($classinfo['prop']))
               			{
//               				echo "<pre>";
//               				print_r($classinfo['prop']);
//               				exit();
               				$count=0;
               				foreach ($classinfo['prop'] as $prop)
               				{
               					if ($prop['posicao']>$posicaoMaior)
               						$posicaoMaior=$prop['posicao'];
               		?>
               				<div id='DivProp<? echo $count?>'>
								<table border='0' width="540" cellpadding='0' cellspacing='6'>
               						<tr>
               							<TD width="50" nowrap class="pblTextoLabelForm">
               							Propriedade <? echo $count?> (<? echo $prop['cod_propriedade'] ?>)
               							</TD>
               							<TD colspan="3">
									...
									<tr>
               							<td class='pblTextoLabelForm'>
               								Prefixo<br>
               								<input type="hidden" name="prop_<? echo $count?>_nomeatual" value="<? echo $prop['nome']?>">
                   							<input class="pblInputForm" type="text" name="prop_<? echo $count?>_nome" value="<? echo $prop['nome']?>">
										</td>
										<td class="pblTextoLabelForm">
											R&oacute;tulo<br>
											<input class="pblInputForm" type="text" name="prop_<? echo $count?>_rotulo" value="<? echo $prop['rotulo']?>"> 
										</td>
                                        <td class='pblTextoLabelForm'>
                                     Tipo de Dado<br>
   											<input type="hidden" name="prop_<? echo $count?>_cod_tipodado" value="<? echo $prop['cod_tipodado']?>">
   											<select disabled class="pblInputForm" name="prop_<? echo $count?>_tipodado" OnChange="ChecaTipoDado(this,<? echo $count?>,trPropClass<? echo $count?>)">
   												<? echo $_page->_administracao->DropDownTipoDado($_page, $prop['cod_tipodado'],true);?>
   											</select>
										</td>
									</tr>
									<tr id="trPropClass<? echo $count?>" style="display:<?= ($prop['cod_tipodado']==6)?'':'none'?>">
									    <td class='pblTextoLabelForm pblObrigatorio'>                               
										Refer&ecirc;ncias:
   										</td>
									
               							<td valign="top" class='pblTextoLabelForm'>
               								<?
               									//echo $prop['cod_referencia_classe'];
               								...
               								else
               								{
               								 	$txtref='';
               								?>�rio
               									<script>ArrayCheck[<? echo $count?>]=1</script>
               								<?
               								}
               								?>
               								Classe de Refer&ecirc;ncia<BR>
               								<input type="hidden" name="prop_<? echo $count?>_cod_referencia_classe" value="<? echo $prop['cod_referencia_classe']?>">
               								<select  disabled  class="pblSelectForm" name="prop_<? echo $count?>_cod_referencia_classe" <? echo $txtref?>>
               									<? echo $_page->_administracao->DropDownClasses($_page, $prop['cod_referencia_classe'],true);?>
               								</select>
               							</td>
               							<td valign="top" class='pblTextoLabelForm'>
               								Campo de Refer&ecirc;ncia<BR>
               								<input type="hidden" name="prop_<? echo $count?>_cod_referencia_campo_ref" value="<? echo $prop['campo_ref']?>">
               								<input  disabled class="pblInputForm" type="text" name="prop_<? echo $count?>_campo_ref" value="<? echo $prop['campo_ref']?>"  <? echo $txtref?> > 
           								</td>
					           							</tr>
									<tr id="trPropClassBlolean<? echo $count?>" style="display:<?= ($prop['cod_tipodado']==2)?'':'none'?>">
										<td class="pblTextoLabelForm pblObrigatorio">Nomea&ccedil;&atilde;o:</td>
										<td valign="top" class="pblTextoLabelForm">
											Campo 1:<BR>
											<input class="pblInputForm" type="text" name="prop_<? echo $count?>_bolean_1" value="<?=$prop['rot1booleano']?>">
										</td>
										<td valign="top" class="pblTextoLabelForm">
											Campo 0:<BR>
											<input class="pblInputForm" type="text" name="prop_<? echo $count?>_bolean_2" value="<?=$prop['rot2booleano']?>">
										</td>
									</tr>
           							<tr>
           							<td valign="top" class='pblTextoLabelForm'>
           							Valor Padr&atilde;o<br>
									<input class="pblInputForm" type="text" name="prop_<? echo $count?>_valorpadrao"
               								 <?= ($prop['valorpadrao']!=" ")?' value="'.$prop['valorpadrao'].'"':'value=""';?> maxlength="200">	
           							</td>
           							<td valign="top" class='pblTextoLabelForm'>
           							Posi&ccedil;&atilde;o<br>
									<input class="pblInputForm" type="text" name="prop_<? echo $count?>_posicao" value="<? echo $prop['posicao']?>">
									</td>
   								<td class="pblTextoLabelForm">
   								Seguran&ccedil;a:
        						<select class="pblSelectForm" name="prop_<? echo $count?>_seguranca" OnChange="">
									...
									<option value="<?=_PERFIL_ADMINISTRADOR?>"<?= ($prop['seguranca']==_PERFIL_ADMINISTRADOR)?' selected ':''?>>Administrador</option>
           							...
           							<tr>
           								<td class="pblTextoLabelForm" colspan="2">
           									...
           									<textarea class="pblInputForm" name="prop_<? echo $count?>_descricao" rows="2" cols="55"><? echo $prop['descricao']?></textarea>
           								</td>
           								<td class="pblTextoLabelForm">
           									<input type="checkbox" name="prop_<? echo $count?>_obrigatorio" <?= ($prop['obrigatorio'])?' checked ':''?> value="1">&nbsp;Obrigat&oacute;rio
           								</td>
           							</tr>
           							<tr>
				...
				<table border=0 width="100%" cellpadding="4" cellspacing="0">
               	...
               	<?
					...
					<tr>
						...
						<td class="pblTextoLabelForm">
							Tipo de Dado<br>
							<select class="pblSelectForm" name="prop_\'+conta_prop+\'_cod_tipodado" OnChange="ChecaTipoDado(this,\'+conta_prop+\',trPropClass\'+conta_prop+\')">
						'. $_page->_administracao->DropDownTipoDado($_page, "",true) .'
							</select>
						</td>
					</tr>
					<tr id="trPropClass\'+conta_prop+\'" style="display:none">
						<td class="pblTextoLabelForm pblObrigatorio">Refer&ecirc;ncias:</td>
						<td valign="top" class="pblTextoLabelForm">
							Classe de Refer&ecirc;ncia<BR>
							<select class="pblSelectForm" name="prop_\'+conta_prop+\'_cod_referencia_classe" disabled>
								'. $_page->_administracao->DropDownClasses($_page, "",true) .'
							</select>
						</td>
						<td valign="top" class="pblTextoLabelForm">
							Campo de Refer&ecirc;ncia<BR>
							<input class="pblInputForm" type="text" name="prop_\'+conta_prop+\'_campo_ref" value="" disabled>
						</td>
					</tr>
					<tr id="trPropClassBlolean\'+conta_prop+\'" style="display:none">
						...
						<td valign="top" class="pblTextoLabelForm">
							Campo 1:<BR>
							<input class="pblInputForm" type="text" name="prop_\'+conta_prop+\'_bolean_1" value="" disabled>
						</td>
						<td valign="top" class="pblTextoLabelForm">
							Campo 0:<BR>
							<input class="pblInputForm" type="text" name="prop_\'+conta_prop+\'_bolean_2" value="" disabled>
						</td>
					</tr>
					<tr>
						<td valign="top" class="pblTextoLabelForm">
							Valor Padr&atilde;o<br>
							<input class="pblInputForm" type="text" name="prop_\'+conta_prop+\'_valorpadrao" value="" maxlength="200">	
						</td>
						<td class="pblTextoLabelForm">
							Posi&ccedil;&atilde;o<br>
							<input class="pblInputForm" type="text" name="prop_\'+conta_prop+\'_posicao" value="">
						</td>
						<td class="pblTextoLabelForm">
							Seguran&ccedil;a:<br>
							<select class="pblSelectForm" name="prop_\'+conta_prop+\'_seguranca" OnChange=trMensagemTemporaria.style.display="">
					...
					<tr>
						<td class="pblTextoLabelForm" colspan="2">
							Descri&ccedil;&atilde;o<br>
							<textarea name="prop_\'+conta_prop+\'_descricao" rows="2" cols="55" class="pblInputForm"></textarea>
						</td>
						<td class="pblTextoLabelForm">
							<input type="checkbox" name="prop_\'+conta_prop+\'_obrigatorio" value="1">&nbsp;Obrigat&oacute;rio
						</td>
					</tr>
					<tr>
						<td colspan="3" valign="bottom" align="right" class="pblTextoForm">
...
<script>
	...
	var conta_prop=<? echo ($count+0)?>;
	var posicaoMaior = <? echo ($posicaoMaior+1);?>;
	<?
	//javascript antigo (substituido por Ma�ra, 11/08/04)
	/*
		function AdicionarPropriedade()
		{
			document.all.pp.innerHTML=document.all.pp.innerHTML+'<? echo $novo_div>';
			document.all['prop_'+conta_prop+'_posicao'].value=posicaoMaior++;
			conta_prop++;
		}
		function ApagarPropriedade(indice)
		{
			document.all['DivProp'+indice].style.visibility="hidden";
			document.all['DivProp'+indice].style.display="none";
			document.all['DivProp'+indice].innerHTML="";
			conta_prop--;
			posicaoMaior--;
		}
	*/
